...
**Conversión de unidades en la conciliación**: pedido y albarán pueden facturar en magnitudes distintas ("20 ud" pedidas vs "7,4 kg" servidos). La conciliación convierte lo recibido a la unidad del pedido (`reception-unit-conversion.util.ts`): misma magnitud por factores (kg↔g, L↔ml); cruce ud↔kg/L mediante el peso/volumen medio por unidad del artículo (`Product.avgUnitWeight`, en la base de la magnitud). Si aún no existe, se deriva del ratio de precios del propio cruce (€/ud ÷ €/kg) y se persiste una sola vez — nunca se sobrescribe automáticamente; edición manual en el artículo. Sin puente fiable (sin peso y sin precios), comportamiento histórico: cantidad cruda ÷ `unitsPerFormat`. La línea guarda la cantidad/unidad cruda del albarán (`receivedSourceQuantity`/`receivedSourceUnit`) para mostrar la equivalencia. Cobertura con tolerancias: ±10% en líneas cruzadas ud↔kg/L (el peso medio arrastra redondeo), ±2%+0,01 en peso/volumen, exacto en uds.
