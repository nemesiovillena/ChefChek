...
- **Visión**: `dashboard`
- **Acceso por rol** (2026-08-30): `role-access` — capa de visibilidad por rol (`USER`/`VIEWER`) por tenant, encima del sistema de módulos. `SectionAccessGuard` + `@RequireSection(...)`, config en tabla `Configuration` (`roleAccess.{ROLE}.{sectionKey}`), pantalla "Permisos por rol" en Configuración (OWNER/ADMIN). Sub-capacidades: `recipes.cost/ficha/edit`, `production.tasks`. Filtra el Asistente IA (tools de coste) y los KPIs del dashboard. Frontend: `useSectionAccess()` (`canSee(key)`), `nav-config.ts` con `sectionKey`. Detalle: `docs/authorization-model.md`.
- **Calidad turística SICTED** (2026-09-24 a 2026-09-26, tenant-activable, `@RequireModule("sicted")`, independiente de `appcc`): registros digitales inalterables (limpieza, apertura/cierre, mantenimiento), proveedores, personas, cliente, y Dirección (catálogo de 144 buenas prácticas del manual real + autoevaluación 1-5, plan de mejora, objetivos anuales, eventos, documentos legales con aviso de caducidad, informe anual agregado). Motor de registros compartido con `appcc` en el módulo neutro `checklists` (triggers Postgres de inalterabilidad — `forbid_mutation`/`forbid_milestone_rewrite`/uno condicional al estado de la tabla padre). Detalle completo: `docs/sicted-system-architecture.md`; plan: `plans/260921-2256-sicted-calidad-turistica/`.
