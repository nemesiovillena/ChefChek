...
describe("AlbaranesService", () => {
  ...
  describe("checkDuplicate", () => {
    it("returns null when supplierId is missing", async () => {
      const result = await service.checkDuplicate("t1", undefined, "A-1");
      expect(result).toBeNull();
      expect(prisma.albaran.findFirst).not.toHaveBeenCalled();
    });

    it("returns null when albaranNumber is empty or only whitespace", async () => {
      expect(await service.checkDuplicate("t1", "s1", "")).toBeNull();
      expect(await service.checkDuplicate("t1", "s1", "   ")).toBeNull();
      expect(prisma.albaran.findFirst).not.toHaveBeenCalled();
    });

    it.each(["MANUAL-123", "OCR-456", "FALLBACK-789"])(
      "returns null for synthetic autogenerated numbers (%s)",
      async (synthetic) => {
        const result = await service.checkDuplicate("t1", "s1", synthetic);
        expect(result).toBeNull();
        expect(prisma.albaran.findFirst).not.toHaveBeenCalled();
      },
    );

    it("queries by tenant, supplier and trimmed case-insensitive number", async () => {
      prisma.albaran.findFirst.mockResolvedValue(null);

      await service.checkDuplicate("t1", "s1", "  A-1  ");

      expect(prisma.albaran.findFirst).toHaveBeenCalledWith({
        where: {
          tenantId: "t1",
          supplierId: "s1",
          albaranNumber: { equals: "A-1", mode: "insensitive" },
        },
        select: {
          id: true,
          albaranNumber: true,
    ...
    it("excludes the given id (edit scenario, avoids self-match)", async () => {
      prisma.albaran.findFirst.mockResolvedValue(null);

      await service.checkDuplicate("t1", "s1", "A-1", "alb-1");

      const { where } = prisma.albaran.findFirst.mock.calls[0][0];
      expect(where.id).toEqual({ not: "alb-1" });
    });

    it("returns the existing albaran when a match is found", async () => {
      const existing = {
        id: "alb-old",
        albaranNumber: "A-1",
        date: new Date("2024-01-01"),
      ...
      prisma.albaran.findFirst.mockResolvedValue(existing);

      const result = await service.checkDuplicate("t1", "s1", "A-1");
