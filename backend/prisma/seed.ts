...
async function main() {
  ...
  // ── L0: SUPERADMIN (sin tenant) ───────────────────────────────────────
  await prisma.user.upsert({
    where: { id: "superadmin-chefchek-system" },
    update: {},
    create: {
      id: "superadmin-chefchek-system",
  ...
  console.log("✅ SUPERADMIN (superadmin@chefchek.io)");
  ...
  for (const s of subs) {
    const c = await prisma.category.create({
      data: {
        ...
        context: "articles",
        parentId: parentBySlug[s.parent],
  ...
  console.log(
    `✅ ${parents.length} parent + ${subs.length} sub article categories`,
  );
  ...
  const supplierDefs = [
    ...
    {
      name: "Pescaderías Mar Azul",
      cifNif: "B56789012",
      contactPerson: "Pedro Sánchez",
      email: "pedro@marazul.com",
  ...
  const suppliers = await Promise.all(
    supplierDefs.map((s) =>
      prisma.supplier.create({
        data: { tenantId: tenant.id, isActive: true, ...s },
      }),
    ),
  );
  const supplierByName = Object.fromEntries(
    suppliers.map((s, i) => [supplierDefs[i].name, s.id]),
  );
  ...
  const productDefs: ProductDef[] = [
    {
      name: "Tomate Pera",
      description: "Tomate fresco de huerta",
      cat: "verduras",
    ...
    {
      name: "Arroz Bomba",
      description: "Arroz D.O. para paellas",
      cat: "arroz-pasta",
      ...
      yieldFactor: 0.97,
      allergens: [],
      altFormat: { name: "Paquete 1kg", format: "1kg", price: 2.5 },
      nutritional: {
        energyKcal: 130,
        protein: 2.7,
        fat: 0.3,
    ...
    },
    {
      ...
      purchaseFormat: "Caja 20kg",
      referenceUnit: "kg",
      unitsPerFormat: 1,
      referenceUnitSize: 20,
      purchasePrice: 3.25,
      netPrice: 3.48,
      profitMargin: 7,
      waste: 8,
    ...
    },
    {
      ...
      description: "AOVE primera presión en frío",
      cat: "aceite",
      ...
      yieldFactor: 0.98,
      allergens: [],
      barcode: "8412345000017",
    },
    {
      name: "Azafrán en Hebras",
      description: "Azafrán D.O. La Mancha",
      cat: "condimentos",
      supplier: "Proveedor Local S.L.",
      purchaseFormat: "Estuche 1g",
      referenceUnit: "g",
      unitsPerFormat: 1,
      referenceUnitSize: 1,
      purchasePrice: 12.0,
    ...
    {
      name: "Lomo de Salmón",
      description: "Salmón noruego congelado",
      cat: "pescado-congelado",
      supplier: "Pescaderías Mar Azul",
      purchaseFormat: "Caja 10kg",
      referenceUnit: "kg",
      unitsPerFormat: 1,
      referenceUnitSize: 10,
    ...
    },
    {
      ...
      description: "Crianza D.O.Ca. Rioja",
      cat: "vino",
      supplier: "Proveedor Local S.L.",
      purchaseFormat: "Caja 6x75cl",
      referenceUnit: "L",
      unitsPerFormat: 6,
      referenceUnitSize: 0.75,
      purchasePrice: 4.2,
      netPrice: 4.5,
      profitMargin: 7,
      waste: 0,
      yieldFactor: 1,
      allergens: [12],
      barcode: "8412345000024",
      altFormat: { name: "Botella 75cl", format: "75cl", price: 6.5 },
    },
    {
      name: "Queso Manchego",
      description: "Queso curado D.O. La Mancha",
      cat: "lacteos",
      supplier: "Proveedor Local S.L.",
      purchaseFormat: "Pieza 3kg",
      referenceUnit: "kg",
      unitsPerFormat: 1,
      referenceUnitSize: 3,
      purchasePrice: 14.0,
    ...
    },
    {
      ...
      description: "Sémola de trigo duro",
      cat: "arroz-pasta",
      supplier: "Arrocerías del Sur",
      purchaseFormat: "Paquete 5kg",
      referenceUnit: "kg",
      unitsPerFormat: 1,
      referenceUnitSize: 5,
      purchasePrice: 1.2,
  ...
  for (const p of productDefs) {
    ...
    if (p.nutritional) {
      await prisma.nutritionalInfo.create({
        data: {
          productId: prod.id,
          energyKcal: p.nutritional.energyKcal,
          protein: p.nutritional.protein,
          fat: p.nutritional.fat,
          carbohydrates: p.nutritional.carbohydrates,
          salt: p.nutritional.salt,
        },
      });
    }
    if (p.altFormat) {
      await prisma.purchaseFormat.create({
        data: {
          productId: prod.id,
          name: p.altFormat.name,
          format: p.altFormat.format,
          price: p.altFormat.price,
        },
      });
    }
  }
  console.log(
    `✅ ${productDefs.length} products (+nutritional +purchaseFormat)`,
  );
  ...
  for (const s of stockDefs) {
    await prisma.stock.create({
      data: {
        tenantId: tenant.id,
        productId: productByName[s.product],
        warehouseId: warehouse.id,
        quantity: s.quantity,
        minimumStock: s.minimum,
        reorderLevel: s.reorder,
        maximumStock: s.reorder * 3,
      },
    });
  }
  ...
  const recipeDefs: RecipeDef[] = [
    {
      ...
      cost: 15.8,
      costPerUnit: 2.1,
      categories: ["arroces", "carnes-recetas"],
      allergens: [],
      ingredients: [
      ...
      translation: {
        ...
        description: "Traditional paella with chicken and vegetables",
      },
    },
    {
      name: "Salmón a la Plancha",
      description: "Lomo de salmón con aceite de oliva",
      portions: 4,
      portionSize: 180,
      cost: 9.2,
      costPerUnit: 2.3,
      ...
      translation: {
        language: "en",
        name: "Grilled Salmon",
        description: "Salmon fillet with olive oil",
      },
    },
    {
      name: "Spaghetti al Tomate",
      description: "Pasta con salsa de tomate fresco",
      portions: 4,
      portionSize: 300,
      cost: 4.5,
      costPerUnit: 1.13,
    ...
    {
      name: "Tabla de Queso Manchego",
      description: "Queso curado con aceite de oliva",
      portions: 6,
      portionSize: 80,
      cost: 6.0,
      costPerUnit: 1.0,
  ...
  for (const r of recipeDefs) {
      ...
      data: {
        ...
        elaboration: JSON.stringify({
          ...
          content: [
            {
              type: "paragraph",
              content: [{ type: "text", text: `Elaboración de ${r.name}.` }],
            },
          ],
        }),
        totalCost: r.cost,
    ...
    for (const ing of r.ingredients) {
      await prisma.recipeIngredient.create({
        data: {
          recipeId: rec.id,
          productId: productByName[ing.product],
    ...
    for (const cat of r.categories) {
      await prisma.recipeCategory.create({
        data: { recipeId: rec.id, categoryId: recipeCatBySlug[cat] },
      });
    }
    if (r.translation) {
  ...
  console.log(
    `✅ ${recipeDefs.length} recipes (+ingredients +categories +translations)`,
  );
  ...
  const sections = [
    {
      name: "Entrantes",
      sortOrder: 1,
      items: [{ recipe: "Tabla de Queso Manchego", price: 9.5, cost: 3.0 }],
    },
    {
      name: "Principales",
      sortOrder: 2,
  ...
  for (const s of sections) {
    ...
    for (const [i, it] of s.items.entries()) {
      await prisma.menuSectionItem.create({
        data: {
          sectionId: sec.id,
          recipeId: recipeByName[it.recipe],
          order: i + 1,
          price: it.price,
          cost: it.cost,
          margin: it.price - it.cost,
          isAvailable: true,
        },
      });
    }
  ...
  await prisma.menuItem.create({
    data: {
      menuId: menu.id,
      recipeId: recipeByName["Paella Valenciana"],
  ...
  await prisma.menuTranslation.create({
    data: {
      ...
      title: "Seasonal Menu",
      name: "Seasonal Menu",
      description: "Week's selection of dishes",
    },
  });
  ...
  for (const l of alb1Lines) {
    ...
    await prisma.stockMovement.create({
      data: {
        productId: productByName[l.product],
        warehouseId: warehouse.id,
        type: "ENTRANCE",
        quantity: l.qty,
        unit: l.unit,
        reason: `Albarán ${albaran1.internalNumber}`,
      },
    });
  }
  ...
  await prisma.stockMovement.create({
    data: {
      productId: productByName["Pollo de Corral"],
      warehouseId: warehouse.id,
      type: "ENTRANCE",
      quantity: 20,
      unit: "kg",
      reason: `Albarán ${albaran2.internalNumber}`,
    },
  });
  ...
  const albaran3 = await prisma.albaran.create({
    data: {
      ...
      ocrConfidence: 0.58,
      ocrRawData: {
        raw_text: "Factura ... Molino desconocido ... harina ... levadura",
      },
      warehouseId: warehouse.id,
      createdBy: admin.id,
    },
  ...
  await Promise.all([
    prisma.albaranLine.create({
      data: {
        albaranId: albaran3.id,
        description: "Harina de trigo especial",
        quantity: 5,
        unit: "kg",
        unitPrice: 0.9,
        vatPercent: 10,
  ...
  await prisma.goodsReception.create({
    data: {
      ...
      fecha: iso(-2),
      items: [
        { product: "Tomate Pera", quantity: 2, unit: "Caja 10kg" },
        { product: "Cebolla", quantity: 1, unit: "Saco 25kg" },
      ],
  ...
  console.log(
    "✅ 3 albaranes (2 confirmados + 1 OCR pendiente) + goods reception + invoice + purchase order",
  );
  ...
  for (const it of invItems) {
    await prisma.inventoryItem.create({
      data: {
        ...
        productId: productByName[it.product],
        quantity: it.actual,
        unit: it.unit,
        theoreticalQuantity: it.theoretical,
        actualQuantity: it.actual,
        difference: it.actual - it.theoretical,
        condition: "GOOD",
      },
    });
  }
  ...
  for (const [ctrl, temps] of [
    [fridge.id, [4.1, 3.8, 5.2]],
    [freezer.id, [-18.5, -19.0, -17.2]],
  ] as [string, number[]][]) {
    for (let d = 0; d < 3; d++) {
      const t = temps[d];
      const within =
        Math.abs(t - (ctrl === fridge.id ? 4 : -18)) <=
        (ctrl === fridge.id ? 2 : 3);
  ...
  const cleaningPlan = await prisma.cleaningPlan.create({
    data: {
      ...
      description: "Rutina de limpieza al cierre",
      area: "Cocina",
      planType: "DAILY",
      frequency: "Diaria",
  ...
  for (const tn of [
    ...
    "Desinfectar superficies de corte",
    "Vaciar y limpiar freidora",
  ]) {
    await prisma.cleaningTask.create({
      data: {
        planId: cleaningPlan.id,
        taskName: tn,
        taskType: "CLEANING",
        area: "Cocina",
        products: ["Desinfectante multiusos"],
        estimatedTime: 15,
        responsible: [admin.name],
        completed: tn !== "Vaciar y limpiar freidora",
        completedBy: tn !== "Vaciar y limpiar freidora" ? admin.id : null,
        completedAt: tn !== "Vaciar y limpiar freidora" ? iso(-1) : null,
      },
    });
  }
  await prisma.pestControl.create({
    data: {
      ...
      area: "Almacén seco",
      empresa: "Desinsecta S.L.",
      company: "Desinsecta S.L.",
      technician: "Externo",
      products: { products: ["Cebos roedores"] },
  ...
  await prisma.appccControl.create({
    data: {
      ...
      unit: "check",
      status: "OK",
      notes: "Recepción de mercancía conforme",
      controlledBy: admin.id,
    },
  });
  console.log(
    "✅ APPCC: 2 temp controls + 6 mediciones + cleaning plan + 3 tasks + pest control + appcc control",
  );
  ...
  const wikiCat = await prisma.knowledgeCategory.create({
    data: {
      tenantId: tenant.id,
      name: "Procedimientos de cocina",
      slug: "procedimientos-cocina",
      description: "Manuales y SOPs",
      color: "#2196F3",
      icon: "book",
      sortOrder: 1,
      isActive: true,
  ...
  const tag1 = await prisma.knowledgeTag.create({
    data: {
      tenantId: tenant.id,
      name: "Higiene",
      slug: "higiene",
  ...
  const tag2 = await prisma.knowledgeTag.create({
    data: {
      tenantId: tenant.id,
      name: "Recetas",
      slug: "recetas",
        ...
        content: [
          {
            ...
            content: [
              {
                type: "text",
                text: "Verificar temperatura, integridad del embalaje y fecha de caducidad.",
              },
            ],
          },
        ],
    ...
    data: {
      ...
      content: {
        ...
        content: [
          {
            type: "paragraph",
            content: [{ type: "text", text: "Versión inicial." }],
          },
        ],
      },
      changeNote: "Creación",
  ...
  await prisma.knowledgeArticleTag.create({
    data: { articleId: article.id, tagId: tag1.id },
  });
  await prisma.knowledgeArticleTag.create({
    data: { articleId: article.id, tagId: tag2.id },
  });
  ...
  await prisma.documentTemplate.create({
    data: {
      tenantId: tenant.id,
      name: "Plantilla albarán",
      type: "RECEIPT",
      description: "Plantilla base para albaranes",
      content: { sections: ["header", "lines", "totals"] },
      isActive: true,
    },
  });
  await prisma.technicalSheetTemplate.create({
    data: {
      tenantId: tenant.id,
      name: "Ficha técnica estándar",
      type: "STANDARD",
      description: "Ficha técnica de producto",
      layout: {},
      fields: ["nombre", "alergenos", "proveedor"],
      isActive: true,
      createdBy: admin.id,
    },
  });
  ...
  await prisma.alert.create({
    data: {
      ...
      alertType: "LOW_STOCK",
      severity: "WARNING",
      message: "Pollo de Corral bajo mínimo (6 < 8)",
      createdBy: admin.id,
    },
  });
  await prisma.alert.create({
    data: {
      ...
      alertType: "LOW_STOCK",
      severity: "WARNING",
      message: "Pasta Spaghetti bajo mínimo (6 < 4)",
      createdBy: admin.id,
    },
  });
  const metrics: {
    ...
    {
      metricType: "STOCK_ALERT",
      metricName: "low_stock_count",
      period: "DAY",
      offset: 0,
      value: 2,
    },
  ...
  await prisma.dashboardAlert.create({
    data: {
      ...
      severity: "HIGH",
      title: "Stock crítico",
      description: "2 productos bajo mínimo",
      entityId: productByName["Pollo de Corral"],
      entityType: "PRODUCT",
      isResolved: false,
    },
  });
  ...
  await prisma.auditLog.create({
    data: {
      ...
      action: "CREATE",
      entityType: "Albaran",
      entityId: albaran1.id,
      details: { internalNumber: albaran1.internalNumber },
    },
  });
  console.log(
    "✅ staff + wiki (cat+article+version+2tags) + digital-menu + QR + 3 scans + config + 2 templates + alerts + metrics + audit",
  );
  ...
  const sprint = await prisma.sprint.create({
    data: {
      ...
      objectives: JSON.stringify([
        "Cerrar inventario semanal",
        "Revisar mermas",
        "Actualizar carta",
      ]),
      teamMembers: JSON.stringify([
        { id: teamMember.id, name: teamMember.name, role: "DEVELOPER" },
      ]),
  ...
  const taskDefs = [
    {
      title: "Cerrar inventario semanal",
      status: "IN_PROGRESS",
      priority: "HIGH",
      due: 4,
    },
    {
      title: "Revisar mermas de carne",
      status: "COMPLETED",
      priority: "MEDIUM",
      due: 2,
    },
    {
      title: "Actualizar carta de temporada",
      status: "TODO",
      priority: "LOW",
      due: 5,
    },
  ...
  for (const t of taskDefs) {
    ...
    await prisma.notification.create({
      data: {
        type: "TASK",
        title: t.title,
        message: `Tarea ${t.status.toLowerCase()}: ${t.title}`,
        taskId: task.id,
        sprintId: sprint.id,
  ...
  console.log(
    "📝 Login: admin@chefchek.local / admin123 (tenant: chefchek-demo)",
  );
