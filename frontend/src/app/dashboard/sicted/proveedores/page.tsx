'use client';

import { useState } from 'react';
import { useRouter } from 'next/navigation';
import { ArrowLeft, ClipboardList, Package, ShieldCheck, Warehouse } from 'lucide-react';
import { useAuth } from '@/contexts/auth.context';
import { SictedSuppliersComplianceTab } from '../components/sicted-suppliers-compliance-tab';
import { SictedSuppliersIncidentsTab } from '../components/sicted-suppliers-incidents-tab';
import { SictedSuppliersInventoryTab } from '../components/sicted-suppliers-inventory-tab';
import { SictedSuppliersEvidenceTab } from '../components/sicted-suppliers-evidence-tab';

export const dynamic = 'force-dynamic';

type TabId = 'cumplimiento' | 'incidencias' | 'inventario' | 'evidencia';

const TABS: { id: TabId; label: string; icon: typeof ShieldCheck }[] = [
  { id: 'cumplimiento', label: 'Perfil de cumplimiento', icon: ShieldCheck },
  { id: 'incidencias', label: 'Incidencias', icon: ClipboardList },
  { id: 'inventario', label: 'Inventario', icon: Warehouse },
  { id: 'evidencia', label: 'Evidencia', icon: Package },
];

/** Proveedores y aprovisionamiento (PROV): perfil de cumplimiento, incidencias de recepción, sello de inventario, panel de evidencia. */
export default function SictedProveedoresPage() {
  const router = useRouter();
  const { isLoading: authLoading } = useAuth();
  const [activeTab, setActiveTab] = useState<TabId>('cumplimiento');

  if (authLoading) return null;

  return (
    <div className="px-margin-mobile md:px-margin-desktop max-w-container-max-width mx-auto pb-24 pt-8">
      <button
        type="button"
        onClick={() => router.push('/dashboard/sicted')}
        className="mb-4 flex min-h-[40px] items-center gap-2 text-[var(--on-surface-variant)]"
      >
        <ArrowLeft className="h-4 w-4" />
        Volver a SICTED
      </button>
      <h2 className="font-headline-lg text-headline-lg text-primary mb-6">Proveedores y aprovisionamiento</h2>

      {/* Tab bar con role=tablist: globals.css oculta <nav> no-fixed, no usar <nav> */}
      <div
        role="tablist"
        aria-label="Secciones de proveedores y aprovisionamiento"
        className="mb-6 flex gap-1 overflow-x-auto rounded-2xl bg-[var(--surface-container)] p-1"
      >
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            role="tab"
            aria-selected={activeTab === id}
            onClick={() => setActiveTab(id)}
            className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium transition ${
              activeTab === id
                ? 'bg-[var(--primary)] text-primary-foreground'
                : 'text-[var(--on-surface-variant)] hover:bg-[var(--surface-container-high)]'
            }`}
          >
            <Icon className="h-4 w-4" />
            {label}
          </button>
        ))}
      </div>

      {activeTab === 'cumplimiento' && <SictedSuppliersComplianceTab />}
      {activeTab === 'incidencias' && <SictedSuppliersIncidentsTab />}
      {activeTab === 'inventario' && <SictedSuppliersInventoryTab />}
      {activeTab === 'evidencia' && <SictedSuppliersEvidenceTab />}
    </div>
  );
}
